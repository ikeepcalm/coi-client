...
public final class ActingState {
    ...
    public static void handle(String json) {
        ...
        try {
            ...
            pathway = Pathways.normalizePathway(JsonRead.string(root, "pathway"));
            ...
            overflowEligible = JsonRead.bool(root, "overflowEligible");
            limited = JsonRead.bool(root, "limited");
            outer = JsonRead.bool(root, "outer");
            hasData = true;
            lastPacketMs = System.currentTimeMillis();

            int granted = JsonRead.intOf(root, "granted");
            if (granted > 0) {
                lastGrant = granted;
                lastGrantSource = JsonRead.string(root, "source");
                lastGrantMs = lastPacketMs;
            }
        } catch (Exception e) {
            CoiLog.LOG.warn("Malformed acting payload: {}", json);
        }
    }
    ...
    public static void debugGrant(int percentPoints) {
        if (!hasData) {
            pathway = BeyonderState.hasIdentity() ? BeyonderState.getPathway() : "fool";
            sequence = BeyonderState.getSequence();
            needed = 5000;
            acting = 0;
            outer = false;
            hasData = true;
