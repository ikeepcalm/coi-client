...
public final class BeyonderHealthOverlay {
    ...
    private static boolean render(GuiGraphicsExtractor ctx) {
        ...
        if (player == null || client.gui.hud.isHidden() || HudLayout.editing()
                || !settings.enabled || !settings.showBeyonderHealth
                || !ClientBeyonderState.hasHealthData()) {
            return false;
        }
        ...
        double poolMax = ClientBeyonderState.maxHealth();
        ...
        if (!settings.epilepsyMode) {
            ...
            if (lastDropMs > 0 && sinceDrop < FLASH_MS) flash = 1f - sinceDrop / (float) FLASH_MS;
            if (fraction < LOW_FRACTION) pulse = (float) (0.5 + 0.5 * Math.sin(now * 0.009));
        }
     ...
     * {@link ClientBeyonderState} or the player: the editor runs over a live
