...
public final class ClientResourceState {
    ...
    public static void handle(String json) {
        ...
        try {
            ...
            long ttl = longOf(root, "ttlMs");
            double max = root.has("max") ? root.get("max").getAsDouble() : 0;
            if (!(max > 0)) max = 1;
            double current = Math.clamp(doubleOf(root, "current"), 0, max);
            ...
            entries.put(id, new Entry(
                    ...
                    ttl > 0 ? now + ttl : 0,
                    !"value".equalsIgnoreCase(string(root, "format")),
                    now
            ));
        } catch (Exception e) {
            System.err.println("COI Client: malformed resource payload: " + json);
        }
    }
    ...
    private static double doubleOf(JsonObject root, String key) {
    ...
    private static long longOf(JsonObject root, String key) {
        return root.has(key) && !root.get(key).isJsonNull() ? root.get(key).getAsLong() : 0;
    }
