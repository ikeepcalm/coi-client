...
/**
 * The hand-written menu document behind the F8 debug screen's <em>Menu</em>
 * button.
 * <p>
 * Vocabulary v2 in one screen: a hero, disclosure rows, a step rail, chip facts
 * and two panel grids, alongside every v1 type and every extension field. It is
 * deliberately longer than any real menu — this is the only harness that exists
 * with no server attached, so a reviewer has to be able to judge the whole
 * vocabulary from it.
 * <p>
 * It is kept as raw JSON, not as built records, so feeding it to the debug
 * screen exercises {@code MenuParser} as well as the renderer.
 */
final class MenuSample {
