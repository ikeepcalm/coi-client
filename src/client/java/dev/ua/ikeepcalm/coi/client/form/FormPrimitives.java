...
public interface FormPrimitives {
    ...
    /**
     * An axis-aligned cube centred on {@code (cx, cy, cz)} with the given half-extent — the
     * overwhelmingly common shape in this package, where nearly every particle, spark, scale, eye,
     * blob and worm segment is a small cube placed at a computed point.
     */
    ...
    /**
     * A straight connector between two points, drawn as the axis-aligned box that bounds them both
     * inflated by {@code radius}. It does not rotate to follow the line — for the stubby, mostly
     * axis-aligned limbs, beams, clock hands and lightning arcs that use it, the bounding box reads
     * as the segment itself and costs one box instead of an oriented cuboid's worth of trigonometry.
     */
    ...
    /**
     * The inner shell of a layered body part: the same box shrunk by {@code inset} on every axis, so
     * a form can stack a brighter core inside a translucent outer skin (fire, light, fog, scales).
     *
     * <p>Skipped entirely when the box is too small to inset — an over-inset box would turn inside
     * out and draw with its faces reversed. Several forms pulse their limbs narrower over time and
     * genuinely cross that threshold mid-animation, so the guard belongs here rather than at the
     * call sites that happen to remember it.
     */
