...
/**
 * The mod's own resource-pack data: the pathway archive, indexed so a potion
 * ingredient's tooltip can say which pathway and sequence wants it.
 * <p>
 * It doubles as the reload hook for everything that caches "does this pack
 * define X?" — ability item models, the plate's gauge symbols and the GUI
 * glyphs all have to forget what they learned when the pack changes, and doing
 * it here is what keeps that from being missed.
 */
public class ClientDataLoader extends SimplePreparableReloadListener<Map<Identifier, JsonElement>> implements PreparableReloadListener {
    ...
    protected @NonNull Map<Identifier, JsonElement> prepare(ResourceManager resourceManager, @NonNull ProfilerFiller profiler) {
        ...
        resourceManager.listResources(FOLDER_NAME, path -> path.getPath().endsWith(".json")).forEach((location, resource) -> {
            try (Reader reader = resource.openAsReader()) {
                ...
                CoiLog.LOG.warn("Failed to parse client JSON resource {}", location, e);
            }
        });
        ...
        resourceManager.getResource(PATHWAY_DATA).ifPresent(resource -> {
            try (Reader reader = resource.openAsReader()) {
                ...
                CoiLog.LOG.warn("Failed to load pathway data", e);
            }
        });
    ...
    protected void apply(Map<Identifier, JsonElement> prepared, @NonNull ResourceManager resourceManager, @NonNull ProfilerFiller profiler) {
        ...
        CoiLog.LOG.info("Loaded {} client JSON files, {} ingredients indexed", prepared.size(), ingredientLookup.size());
    }
    ...
    /**
     * The archive names colours by their vanilla formatting word. Only the
     * twelve the data actually uses are mapped; anything else is white rather
     * than a guess.
     */
