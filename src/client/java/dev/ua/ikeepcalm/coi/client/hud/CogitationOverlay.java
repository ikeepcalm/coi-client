...
public class CogitationOverlay {
    ...
    private static void render(GuiGraphicsExtractor ctx, DeltaTracker tickCounter) {
        ...
        if (client.player == null || client.gui.hud.isHidden() || HudLayout.editing() || !settings.enabled
                || !settings.showCogitationOverlay || !ClientCogitationState.hasPrompt()) {
            return;
        }
        ...
        drawCardAt(ctx, pos[0], pos[1], ClientCogitationState.getLabel(),
                ClientCogitationState.getStreak(), ClientCogitationState.timeFraction(now),
                ClientCogitationState.isFailing(now));
        HudScale.pop(ctx);
    }
