...
public class SpiritualityHudOverlay {
    ...
    public static void initialize() {
        HudElementRegistry.attachElementBefore(VanillaHudElements.CHAT, SPIRITUALITY_LAYER, SpiritualityHudOverlay::render);
    }
    ...
    private static void render(GuiGraphicsExtractor ctx, DeltaTracker tickCounter) {
        ...
        if (client.player == null || client.gui.hud.isHidden() || HudLayout.editing() || !settings.enabled
                || !settings.showSpiritualityBar || !ClientBeyonderState.hasSpiritualityData()) {
            return;
        }

        int max = Math.max(1, ClientBeyonderState.getMaxSpirituality());
        int current = Math.clamp(ClientBeyonderState.getSpirituality(), 0, max);
        double predicted = Math.clamp(ClientBeyonderState.predictedSpirituality(), 0, max);
        boolean regen = ClientBeyonderState.isSpiritualityRegen();
    ...
    private static void drawFlash(GuiGraphicsExtractor ctx, int x, int y, int fillW, float alpha) {
        float flash = ClientBeyonderState.getSpiritualityFlashIntensity();
        if (flash > 0 && fillW > 0) {
    ...
    private static float regenFlare(boolean regen, boolean epilepsy) {
        ...
        long age = ClientBeyonderState.spiritualityGainAgeMs();
