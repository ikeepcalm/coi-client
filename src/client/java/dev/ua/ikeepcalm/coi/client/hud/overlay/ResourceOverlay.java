...
public final class ResourceOverlay {
    ...
    public static void initialize() {
        HudElementRegistry.attachElementBefore(VanillaHudElements.CHAT, RESOURCE_LAYER, ResourceOverlay::render);
    }
    ...
    private static void render(GuiGraphicsExtractor ctx, DeltaTracker tickCounter) {
        ...
        if (HudGate.blocked(client, settings)
                ...
                || !settings.showResourceBars || !ResourceState.hasData()) {
            return;
        }
        ...
        List<ResourceState.Entry> bars = ResourceState.visible();
    ...
    private static void drawBar(GuiGraphicsExtractor ctx, Minecraft client, ResourceState.Entry entry, int barX, int barY) {
        int rgb = entry.rgb();
    ...
    private static String labelText(ResourceState.Entry entry) {
