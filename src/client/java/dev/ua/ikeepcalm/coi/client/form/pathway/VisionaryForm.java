...
/**
 * Visionary — a bipedal golden dragon. The only form that renders as a partial form: the baked
 * lower-body model in {@link FormModelLayers#VISIONARY_LOWER_SPEC} stands in for the legs, and
 * {@link #render} survives only as a fallback if that model fails to load.
 *
 ...
 */
public final class VisionaryForm implements MythicalCreatureForm {
    ...
    public PartialFormSpec partialForm() {
        return FormModelLayers.VISIONARY_LOWER_SPEC;
    }
    ...
    public void render(AvatarRenderState state, PoseStack.Pose pose, VertexConsumer consumer) {
        ...
        for (int i = 0; i < scaleCount; i++) {
            ...
            float sSize = 0.036f;
            drawCube(pose, consumer, scaleX, scaleH, scaleZ, sSize, 1.0f, 0.92f, 0.45f, 0.75f, light);
        }
    }
    ...
    private void drawScaled(PoseStack.Pose pose, VertexConsumer consumer,
        ...
        drawInsetBox(pose, consumer, x1, y1, z1, x2, y2, z2, 0.042f, 1.0f, 0.92f, 0.45f, 0.34f, light);
    }
}
