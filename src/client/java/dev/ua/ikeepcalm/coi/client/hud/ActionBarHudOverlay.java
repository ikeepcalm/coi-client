 ...
 * untouched.
 */
public class ActionBarHudOverlay {
    ...
    public static void initialize() {
        HudElementRegistry.attachElementBefore(VanillaHudElements.CHAT, ACTION_BAR_LAYER, ActionBarHudOverlay::render);
    }
    ...
    private static void render(GuiGraphicsExtractor ctx, DeltaTracker tickCounter) {
        ...
        if (client.player == null || client.gui.hud.isHidden() || HudLayout.editing()
                || !settings.enabled || !settings.showActionBar) {
            return;
        }

        // 0 = follow whatever the server thinks fits
        int serverMax = ClientActionBarState.getMaxVisible();
        int limit = settings.actionBarLines > 0 ? Math.min(serverMax, settings.actionBarLines) : serverMax;

        List<ClientActionBarState.Entry> entries = ClientActionBarState.visibleEntries(System.currentTimeMillis(), limit);
        ...
        for (int i = 0; i < entries.size(); i++) {
            ClientActionBarState.Entry entry = entries.get(i);
