...
public class TargetHealthOverlay {
    ...
    private static void render(GuiGraphicsExtractor ctx, DeltaTracker tickCounter) {
        ...
        if (client.player == null || client.gui.hud.isHidden() || HudLayout.editing() || !settings.enabled
                || !settings.showTargetHealth || !ClientTargetState.isVisible(now)) {
            return;
        }
    ...
    private static void drawBar(GuiGraphicsExtractor ctx, int barX, int barY, long now) {
        drawBarAt(ctx, barX, barY, ClientTargetState.getName(),
                ClientTargetState.getAfter(), ClientTargetState.getBefore(),
                ClientTargetState.getHealth(), ClientTargetState.getMax(),
                ClientTargetState.isFlashing(now));
    }
