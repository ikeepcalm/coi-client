...
import dev.ua.ikeepcalm.coi.client.form.pathway.AbyssForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.ChainedForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.DarknessForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.DeathForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.DemonessForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.DoorForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.EmperorForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.ErrorForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.FoolForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.FortuneForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.GiantForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.HangedForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.HermitForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.JusticiarForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.ParagonForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.PriestForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.SunForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.TowerForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.TyrantForm;
import dev.ua.ikeepcalm.coi.client.form.pathway.VisionaryForm;
...
/**
 * Tracks which players the server has transformed, and owns the registry of the twenty pathway
 * forms they can be transformed into.
 *
 * <p>Two maps, with different lifetimes: {@code REGISTERED_FORMS} is fixed at class-init and maps
...
public class MythicalFormManager {
    ...
    /**
     * Registers a form under its lower-cased name and under every spacing variant of it, so a
     * server that says {@code "sea god"}, {@code "sea_god"} or {@code "seagod"} all reach the same
     * form without the lookup having to guess which convention the plugin used.
     */
    ...
    /**
     * Applies one {@code coi-client:mythical} packet. {@code params} is
     * {@code pathway:<unused>:start|stop}; anything malformed or empty clears the player's form
     * rather than leaving them stuck mid-transformation.
     */
    ...
    private static void triggerTransformVfx(String targetUuid, String pathway, boolean starting) {
        ...
        for (AbstractClientPlayer player : client.level.players()) {
            ...
            // The mod-wide colour table, so the burst matches the HUD and the picker.
