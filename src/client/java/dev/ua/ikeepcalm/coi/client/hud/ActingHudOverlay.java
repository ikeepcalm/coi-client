...
public class ActingHudOverlay {
    ...
    public static void initialize() {
        HudElementRegistry.attachElementBefore(VanillaHudElements.CHAT, ACTING_LAYER, ActingHudOverlay::render);
    }
    ...
    private static void render(GuiGraphicsExtractor ctx, DeltaTracker tickCounter) {
        ...
        if (client.player == null || client.gui.hud.isHidden() || HudLayout.editing() || !settings.enabled
                ...
                || !settings.showActingBar || !ClientActingState.hasData() || ClientActingState.isOuter()) {
            return;
        }
    ...
    private static void drawBar(GuiGraphicsExtractor ctx, Minecraft client, int barX, int barY) {
        int rgb = AbilityInfo.pathwayRgb(ClientActingState.getPathway());
        double percent = Math.clamp(ClientActingState.getPercent(), 0, 100);
    ...
    private static String labelText(double percent) {
        ...
        if (ClientActingState.cooldownRemainingNow() > 0) {
            text = text + " · " + I18n.get("hud.coi.acting_cooldown", ClientActingState.cooldownClock());
        }
        return text;
    }
